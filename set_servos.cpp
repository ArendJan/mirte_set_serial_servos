...
servo_settings_t servos_settings = {
    #if servo_num ==2
    .id = 2,
    .min = 3400,
    .max = 21000,
...
int main()
{
    ...
    while(true){
            ...
            printf("id: %i\n", id);
            if(id != servos_settings.id){
                bus.id_write(servos_settings.id);
                printf("Set id to %i\n", servos_settings.id);
            } 
            sleep_ms(1000);
